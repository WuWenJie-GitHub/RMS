...
(function(a) {
    a("#inputForm .card-footer [class*=col-sm-offset]").append('<div class="form-error">' + a.validator.messages.errorMessage + "</div>");
    a.extend(a.validator.defaults, {
        ...
        errorPlacement: function(b, c) {
            if (c.closest(".custom-checkbox").length > 0) {
                c = c.closest(".custom-checkbox");
                var t = c.parent().closest(".checkbox-list");
                if(t.length>0){
                    t.parent().css("position", "relative");
                    t.after(b);
                    // t.insertAfter(c);
                    b.css({
                        top: t.position().top + t.outerHeight() + 2,
                        left: t.position().left + 5
                    });
                    return
                }
            }
            if (c.closest(".custom-radio").length > 0) {
                c = c.closest(".custom-radio");
                var t = c.parent().closest(".radio-list");
                if(t.length>0){
                    t.parent().css("position", "relative");
                    t.after(b);
                    b.css({
    ...
    a.extend(a.fn, {
        resetValid: function() {
            var b = a(this),
            c;
            // if (b.closest(".custom-checkbox").length > 0) {
            //     c = b.closest(".custom-checkbox").parent()
            // }else if (b.closest(".custom-radio").length > 0) {
            //     c = b.closest(".custom-radio").parent()
            // }else 
            if (b.closest(".select2").length > 0) {
                c = b.closest(".select2").parent()
            }else {
