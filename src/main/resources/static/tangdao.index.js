...
$(function () {
    ...
    $('.nav-aside li.treeview a').on('click',function(e){
        e.preventDefault();

        var _that = $(this).parent();

        var _that_sub = _that.parent().parent();
        if(_that_sub.hasClass('with-sub')){
            _that_sub.siblings().removeClass('active show');
            _that_sub.addClass('active show');
        }

        if(!_that.hasClass('with-sub')){
            _that.siblings().removeClass('active');
            _that.siblings().removeClass('show');
            $('.nav-aside li.treeview').removeClass('active');
            ...
            if (g && g != "" && g != window.location.pathname) {
                ...
                if (f && f.length > 0) {
                    ...
                    if ($(".nav-aside").length > 0) {
                        js.addTabPage(null, "新标签", g)
                    }
                }
            } else {
                var d = $(".nav-aside > li:eq(0):not(.active) > a:eq(0)");
